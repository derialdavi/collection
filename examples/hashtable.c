...
int main(void)
{
    ...
    rc = coll_hashtable_init(&ht, NULL, NULL);
    if (rc != COLLECTION_OK) { printf("init: %d\n", rc); return 1; }

    /* put: the key and the value are both copied into the table */
    int key   = 1;
    int value = 100;
    rc = coll_hashtable_put(&ht, &key, sizeof(key), &value, sizeof(value));
    ...
    key = 2;
    int out = 0;
    rc = coll_hashtable_get(&ht, &key, sizeof(key), &out, sizeof(out));
    ...
    key = 1;
    size_t value_size = 0;
    rc = coll_hashtable_get_value_size(&ht, &key, sizeof(key), &value_size);
    ...
    printf("every pair:\n");
    while ((rc = coll_hashtable_get_pair(&ht, &key, sizeof(key), &value,
    ...
    rc = coll_hashtable_init(&ht, coll_hashtable_hash_string,
                             coll_hashtable_cmp_string);
    if (rc != COLLECTION_OK) { printf("init: %d\n", rc); return 1; }

    const char *name = "answer";
    value = 42;
    rc = coll_hashtable_put(&ht, name, strlen(name) + 1, &value, sizeof(value));
    if (rc != COLLECTION_OK) { printf("put: %d\n", rc); return 1; }

    rc = coll_hashtable_get(&ht, name, strlen(name), &out, sizeof(out));
    if (rc != COLLECTION_OK) { printf("get: %d\n", rc); return 1; }
    printf("%s -> %d\n", name, out);
