...
/* list_sort() takes a comparison in the same spirit as the one qsort() wants:
   negative when a comes first, positive when b does, 0 when neither */
...
int main(void)
{
    ...
    size_t index = 0;
    value = 10;
    rc = list_find(&l, &value, sizeof(value), &index);
    ...
    /* at: the node at a position, and get_first: the node at the front. Both
       hand back a struct node *, which the library keeps opaque, so it is
       good for telling you an element is there and not for reading it. Use
       list_find() when you want to know where a value is */
    struct node *node = NULL;
    rc = list_at(&l, 1, &node);
    if (rc != COLLECTION_OK) { printf("at: %d\n", rc); return 1; }

    rc = list_at(&l, 99, &node);
    printf("at index 99: %s\n",
           rc == COLLECTION_ERANGE ? "COLLECTION_ERANGE" : "??");

    struct node *first = NULL;
    rc = list_get_first(&l, &first);
    if (rc != COLLECTION_OK) { printf("get_first: %d\n", rc); return 1; }

    /* remove: the first element with these bytes */
    value = 40;
    rc = list_remove(&l, &value, sizeof(value));
    if (rc != COLLECTION_OK) { printf("remove: %d\n", rc); return 1; }

    /* remove_at: by position */
    rc = list_remove_at(&l, 0);
    ...
    /* remove_all: every element with these bytes at once. It is
       ...
       did */
    value = 10;
    rc = list_remove_all(&l, &value, sizeof(value));
    ...
    /* destroy: frees every element. The list_t itself is yours, and it is
    ...
    value = 7;
    rc = list_init(&l, 5, &value, sizeof(value));
    if (rc != COLLECTION_OK) { printf("init: %d\n", rc); return 1; }
