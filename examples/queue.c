...
int main(void)
{
    ...
    /* init: 0 asks for an empty queue. The last two arguments prefill it, and
       are ignored here */
    rc = queue_init(&q, 0, NULL, 0);
    if (rc != COLLECTION_OK) { printf("init: %d\n", rc); return 1; }

    /* is_empty */
    bool empty = false;
    rc = queue_is_empty(&q, &empty);
    ...
    int value = 10;
    rc = queue_enqueue(&q, &value, sizeof(value));
    ...
    size_t elem_size = 0;
    rc = queue_peek_size(&q, &elem_size);
    ...
    /* destroy: frees every element. The queue_t itself is yours, and it is
    ...
    value = 7;
    rc = queue_init(&q, 4, &value, sizeof(value));
    if (rc != COLLECTION_OK) { printf("init: %d\n", rc); return 1; }
