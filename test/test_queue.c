...
/*
 * queue.h publishes struct queue but keeps struct queue_node opaque, so there
 * is no public way to inspect the chain. These tests mirror the definition
 * from queue.c in order to check the links directly.
 *
...
void setUp(void)
{
    /* poison the handle so a field queue_init forgets to write shows up as
...
/* asserts that q is the empty queue queue_init() and queue_destroy() promise */
static void assert_empty(const queue_t *q)
{
...
static void assert_contents(const queue_t *q, const int *expected, size_t count)
{
    ...
    size_t             seen = 0;
    struct queue_node *back = NULL;

    for (struct queue_node *n = q->front; n != NULL; n = n->next)
    {
...
static struct snapshot snapshot_of(const queue_t *q)
{
...
static void assert_unchanged(const queue_t *q, struct snapshot before,
...
static void fill(queue_t *q, const int *values, size_t count)
{
    TEST_ASSERT_EQUAL_INT(COLLECTION_OK, queue_init(q, 0, NULL, 0));

    for (size_t i = 0; i < count; i++)
        TEST_ASSERT_EQUAL_INT(COLLECTION_OK,
                              queue_enqueue(q, &values[i], sizeof(values[i])));
...
{
    TEST_ASSERT_EQUAL_INT(COLLECTION_OK, queue_init(&queue, 0, NULL, 0));
...
void test_queue_should_PrefillEveryElementWithACopyOfTheDefaultValue(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(COLLECTION_OK,
                          queue_init(&queue, count, &def, sizeof(def)));
    ...
    for (struct queue_node *n = queue.front; n != NULL; n = n->next)
    {
        TEST_ASSERT_NOT_NULL(n->data);
...
void test_queue_should_RejectAZeroElementSizeWhenPrefilling(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(COLLECTION_EINVAL, queue_init(&queue, 4, &def, 0));
...
void test_queue_should_ReportEnomemWhenAPrefilledElementCannotBeAllocated(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(COLLECTION_ENOMEM,
                          queue_init(&queue, 1, &def, SIZE_MAX));
...
void test_queue_should_EmptyTheQueueOnDestroyAndLeaveItReusable(void)
{
    ...
    /* usable again without a second queue_init */
    const int again = 42;
    TEST_ASSERT_EQUAL_INT(COLLECTION_OK,
                          queue_enqueue(&queue, &again, sizeof(again)));
...
{
    TEST_ASSERT_EQUAL_INT(COLLECTION_ENULL, queue_destroy(NULL));
}
...
void test_queue_should_EnqueueTheFirstElementAsBothFrontAndBack(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(COLLECTION_OK, queue_init(&queue, 0, NULL, 0));
    TEST_ASSERT_EQUAL_INT(COLLECTION_OK,
                          queue_enqueue(&queue, &value, sizeof(value)));
...
void test_queue_should_EnqueueAtTheBackWithoutDisturbingTheElementsWaiting(void)
{
    ...
    fill(&queue, values, 3);
    struct queue_node *front_before = queue.front;

    TEST_ASSERT_EQUAL_INT(COLLECTION_OK,
                          queue_enqueue(&queue, &enqueued, sizeof(enqueued)));
...
void test_queue_should_RejectNullPointersOnEnqueue(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(COLLECTION_ENULL,
                          queue_enqueue(NULL, &value, sizeof(value)));
    TEST_ASSERT_EQUAL_INT(COLLECTION_ENULL,
                          queue_enqueue(&queue, NULL, sizeof(value)));
    /* elem_size is irrelevant: a null pointer is reported before EINVAL */
    TEST_ASSERT_EQUAL_INT(COLLECTION_ENULL, queue_enqueue(&queue, NULL, 0));
...
void test_queue_should_RejectAZeroElementSizeOnEnqueue(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(COLLECTION_OK, queue_init(&queue, 0, NULL, 0));
    TEST_ASSERT_EQUAL_INT(COLLECTION_EINVAL, queue_enqueue(&queue, &value, 0));
...
void test_queue_should_ReportEnomemWhenAnEnqueuedElementCannotBeAllocated(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(COLLECTION_ENOMEM,
                          queue_enqueue(&queue, &value, SIZE_MAX));
...
void test_queue_should_DequeueElementsInTheOrderTheyWereEnqueued(void)
{
    ...
    for (size_t i = 0; i < 4; i++)
    {
        ...
        TEST_ASSERT_EQUAL_INT(COLLECTION_OK,
                              queue_dequeue(&queue, &out, sizeof(out)));
        /* first in, first out */
...
void test_queue_should_KeepServingElementsEnqueuedWhileItDrains(void)
{
    ...
    const int late = 3;
    TEST_ASSERT_EQUAL_INT(COLLECTION_OK,
                          queue_enqueue(&queue, &late, sizeof(late)));
...
void test_queue_should_CopyOutExactlyTheStoredElementOnDequeue(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(COLLECTION_OK,
                          queue_dequeue(&queue, &out[0], sizeof(out[0])));
...
void test_queue_should_RejectNullPointersOnDequeue(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(COLLECTION_ENULL,
                          queue_dequeue(NULL, &out, sizeof(out)));
    TEST_ASSERT_EQUAL_INT(COLLECTION_ENULL,
                          queue_dequeue(&queue, NULL, sizeof(out)));
    /* elem_size is irrelevant: a null pointer is reported before EINVAL */
    TEST_ASSERT_EQUAL_INT(COLLECTION_ENULL, queue_dequeue(&queue, NULL, 0));
...
void test_queue_should_RejectAZeroElementSizeOnDequeue(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(COLLECTION_EINVAL, queue_dequeue(&queue, &out, 0));
...
void test_queue_should_RejectAnElementSizeThatDoesNotMatchOnDequeue(void)
{
    ...
    int right = 0;
    TEST_ASSERT_EQUAL_INT(COLLECTION_OK,
                          queue_dequeue(&queue, &right, sizeof(right)));
...
void test_queue_should_PeekTheFrontElementWithoutRemovingIt(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(COLLECTION_OK, queue_peek(&queue, &out, sizeof(out)));
...
void test_queue_should_ReportNotfoundWhenPeekingAnEmptyQueue(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(COLLECTION_ENOTFOUND,
                          queue_peek(&queue, &out, sizeof(out)));
...
void test_queue_should_RejectNullPointersOnPeek(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(COLLECTION_ENULL,
                          queue_peek(NULL, &out, sizeof(out)));
    TEST_ASSERT_EQUAL_INT(COLLECTION_ENULL,
                          queue_peek(&queue, NULL, sizeof(out)));
    /* elem_size is irrelevant: a null pointer is reported before EINVAL */
    TEST_ASSERT_EQUAL_INT(COLLECTION_ENULL, queue_peek(&queue, NULL, 0));
...
void test_queue_should_RejectAZeroElementSizeOnPeek(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(COLLECTION_EINVAL, queue_peek(&queue, &out, 0));
...
void test_queue_should_ReportTheSizeOfTheFrontElement(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(COLLECTION_OK, queue_peek_size(&queue, &size));
...
void test_queue_should_TrackTheSizeOfEachFrontElementInAMixedQueue(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(COLLECTION_OK, queue_init(&queue, 0, NULL, 0));
    TEST_ASSERT_EQUAL_INT(COLLECTION_OK,
                          queue_enqueue(&queue, &first, sizeof(first)));
    TEST_ASSERT_EQUAL_INT(COLLECTION_OK,
                          queue_enqueue(&queue, &second, sizeof(second)));
    TEST_ASSERT_EQUAL_INT(COLLECTION_OK,
                          queue_enqueue(&queue, &third, sizeof(third)));
    ...
    int out_first = 0;
    TEST_ASSERT_EQUAL_INT(COLLECTION_OK,
                          queue_dequeue(&queue, &out_first, size));
    ...
    double out_second = 0;
    TEST_ASSERT_EQUAL_INT(COLLECTION_OK,
                          queue_dequeue(&queue, &out_second, size));
    ...
    char out_third = 0;
    TEST_ASSERT_EQUAL_INT(COLLECTION_OK,
                          queue_dequeue(&queue, &out_third, size));
...
void test_queue_should_RejectNullPointersOnPeekSize(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(COLLECTION_ENULL, queue_peek_size(NULL, &size));
    TEST_ASSERT_EQUAL_INT(COLLECTION_ENULL, queue_peek_size(&queue, NULL));
...
void test_queue_should_ReportTheNumberOfElementsItHolds(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(COLLECTION_OK, queue_init(&queue, 0, NULL, 0));
    TEST_ASSERT_EQUAL_INT(COLLECTION_OK, queue_get_size(&queue, &size));
...
void test_queue_should_RejectNullPointersOnGetSize(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(COLLECTION_ENULL, queue_get_size(NULL, &size));
    TEST_ASSERT_EQUAL_INT(COLLECTION_ENULL, queue_get_size(&queue, NULL));
...
void test_queue_should_ReportEmptyOnlyWhileItHoldsNoElements(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(COLLECTION_OK, queue_init(&queue, 0, NULL, 0));
    TEST_ASSERT_EQUAL_INT(COLLECTION_OK, queue_is_empty(&queue, &empty));
...
void test_queue_should_RejectNullPointersOnIsEmpty(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(COLLECTION_ENULL, queue_is_empty(NULL, &empty));
    TEST_ASSERT_EQUAL_INT(COLLECTION_ENULL, queue_is_empty(&queue, NULL));
